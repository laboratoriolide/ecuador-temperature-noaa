...
Si se ejecuta `code/r/4_weather_maps.R`, de manera opcional, se guardan mapas en formato PNG en `img/weather_maps/`.
...
If `code/r/4_weather_maps.R` is executed (optionally), it saves PNG maps in `img/weather_maps/`.
...
- `img/`: Carpeta creada al generar mapas con `code/r/4_weather_maps.R` que almacena las salidas en PNG.

- `img/`: Directory created when maps are generated with `code/r/4_weather_maps.R` that stores the PNG outputs.
